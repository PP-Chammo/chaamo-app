...
const PlaceBidModalContent: React.FC<PlaceBidModalContentProps> = ({
  ...
  const [bid, setBid] = useState(currentBidAmount);
    ...
    <View className={classes.container}>
      ...
      <View className={classes.maxBidRow}>
        ...
        <Row className={classes.inputContainer}>
          <Label className={classes.labelCurrency}>{currencySymbol}</Label>
          <TextInput
        ...
        {Number(currentBidAmount) > 0 && (
          <Label className={classes.currentBid}>
            Current Bid:{' '}
            {formatDisplay(user?.profile?.currency, Number(currentBidAmount))}
          </Label>
        )}
      </View>
...
const classes = {
  container: 'flex-1 gap-4.5 bg-primary-500 rounded-t-3xl pt-4.5 px-4.5',
  headerRow: 'mb-2',
  title: 'text-white font-bold',
  time: 'text-white font-semibold',
  maxBidRow: 'gap-0.5',
  label: 'text-white text-base font-semibold mb-2',
  labelCurrency: 'text-white !text-3xl font-bold -mb-2',
  inputContainer: 'border-b border-white mb-2',
  input: 'flex-1 text-white text-3xl font-bold !pb-1',
